...
import Resume from "../assets/Resume-FaranKhalil.pdf";
...
class Home extends Component {
  ...
  paragraphs = ["Full Stack Developer.", "MERN Stack Developer."];
  ...
  eraseParagraph = () => {
    ...
    this.interval = setInterval(() => {
      if (index > 0) {
        this.setState({
          displayText: this.state.displayText.substring(0, index - 1),
        });
        index--;
      } else {
        clearInterval(this.interval);
        this.setState(
          (prevState) => ({
            currentParagraph:
              (prevState.currentParagraph + 1) % this.paragraphs.length,
            isErasing: false,
          }),
          this.showParagraph
        );
            ...
            <Reveal>
              <div className="flex flex-col items-center justify-center md:ml-64 mt-4 md:mb-52 md:mt-24 md:mr-28">
                ...
                <h1 className="md:text-5xl font-Oswald text-2xl font-bold text-nav-black dark:text-white flex">
                  <span> I'm </span>
                  <span className="text-custom-green md:ml-6 ml-2">
                    Faran Khalil
                  </span>
                </h1>
